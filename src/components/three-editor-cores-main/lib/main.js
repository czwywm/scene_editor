...
export class ThreeEditor {
  ...
  setCss2dDOM(DOM, position) {
    // console.log('setCss2dDOM', DOM, position);
    if (DOM) {
  ...
  setModelFromInfo(rootInfo, params = null) {
    const loaderService = this.viewer.modelControls.sceneInsertModel(
      ThreeEditor.dracoPath,
      rootInfo,
      params,
    )
  ...
  // 打开GUI控制面板
  openControlPanel() {
    return this.viewer.openControlPanel()
  }
  ...
  setGUIDomPosition(DOM) {
    ...
    return {
      GUIDom: this.viewer.GUI.domElement,
      remove: () => DOM.removeChild(this.viewer.GUI.domElement),
    }
  }
  ...
  setModelAnimationPlay(group) {
    ...
    return this.viewer.modelControls.animationPlay(
      this.viewer.MixerList,
      group,
  ...
  removeModelAnimation(group) {
    ...
    this.viewer.MixerList.splice(
      this.viewer.MixerList.findIndex((i) => i == group),
      1,
    )
  }
  ...
  createGsapAnimation(cameraPosition, maxView) {
    // 直接使用GSAP创建平移动画
    return gsap.to(cameraPosition, {
      duration: 2,
      x: maxView.x,
      y: maxView.y,
      z: maxView.z,
