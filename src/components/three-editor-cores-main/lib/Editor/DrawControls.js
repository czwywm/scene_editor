...
export const drawControls = {
	...
	modeList: ['围栏物体', '平面绘制', '曲线路径', '直线路径', '立体描边'],
	...
	sceneEventCall: function (point, object) {
		switch (this.pointMode) {
			...
			case '地图区域中心':
				...
				return this.drawEventCall?.(
					object.parent.getTransformedVector(
						object.geoInfo.properties.centroidCoord3
							.clone()
							.add(object.position)
							.sub(object.initTranslate),
					),
				)
			...
			case '地图行政中心':
				...
				return this.drawEventCall?.(
					object.parent.getTransformedVector(
						object.geoInfo.properties.centerCoord3
							.clone()
							.add(object.position)
							.sub(object.initTranslate),
					),
...
export function setDrawControlsPanel(scene, transformControls, CommonFrameList, folder) {
		...
		.add(
			{
				fn: () => {
					...
					drawControls.currentDrawFolder = folder.addFolder(
					...
					setDrawMeshPanel(
						...
						transformControls,
						CommonFrameList,
						drawControls.currentDrawFolder,
					)
	...
	drawControls.meshList?.forEach((i) => {
		...
		setDrawMeshPanel(
			...
			transformControls,
			CommonFrameList,
			folder.addFolder(mesh.name + mesh.id),
			i,
		)
	})
}
