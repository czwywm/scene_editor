...
threeEditor.viewer.chartsMapControls.geoGroupLoadCall = (group) => {
	...
	document.querySelector('#threeBox').addEventListener('mousemove', (e) => {
		const i = getIntersects(e, group.children).find(
			(i) => i.object.text !== 'TransformControls' && i.object.isMesh,
		)
...
document.querySelector('#threeBox').addEventListener('dblclick', (e) => {
	threeEditor.getSceneEvent(e, (info) => {
		threeEditor
			.setCss2dDOM(document.querySelector('#myDom'), info.point)
...
window.add = (k) => {
	if (k === 0) threeEditor.setSky(skyUrls())
		...
		threeEditor.setModelFromInfo(data[k]).loaderService.progress = (e) => console.log()
		...
		threeEditor.removeModelAnimation(threeEditor.viewer.currentInfo?.currentRootModel)
	else if (k === 24) {
		...
		threeEditor.viewer.currentInfo?.currentRootModel.children.forEach((i) => {
			if (i.geoInfo?.properties?.centroidCoord3) {
				...
				const m = threeEditor.setCss2dDOM(
					dom,
					getTransformedVector(i.geoInfo?.properties.centroidCoord3),
				)
		...
		return Promise.all([
			...
			new Promise((resolve) =>
				threeEditor.setGsapAnimation(controls.target, target, { duration: 2, onComplete: resolve }),
			),
		])
	}
}
...
function listFiles(url) {
	...
	return new Promise(
		(resolve, reject) =>
		(xhr.onload = () =>
			xhr.status === 200 ? resolve(parseFileList(xhr.responseText)) : reject(xhr.statusText)),
	)
}
